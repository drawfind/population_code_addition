...
# This python code demonstrates the Deneve model for computing with
...
num_steps = 2000 # Number of simulation time steps
...
net = model_Deneve.NeuralNetwork(N)
for t in tqdm(range(num_steps)):
    A[:,t] = model_Deneve.create_code(a,a_sig,N)
    B[:,t] = model_Deneve.create_code(b,b_sig,N)
    C[:,t] = net.compute(A[:,t], B[:,t])
...
plt.tight_layout()
plt.savefig('PC_main_sim_spike_trains_deneve.eps', format='eps')
...
plt.savefig('PC_main_sim_code_sample_deneve.eps', format='eps')
...
plt.savefig('PC_main_sim_corr_fire_rates_near_deneve.eps', format='eps')
...
plt.gca().add_patch(ellipseC)
plt.savefig('PC_main_sim_corr_fire_rates_far_deneve.eps', format='eps')
