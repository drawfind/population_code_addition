...
for t in tqdm(range(num_trials),file=sys.stderr):
    ...
    for N in n_values:
        ...
        D = np.zeros((N, num_steps))
        
        for i in range(num_steps):
            A = model.create_code(a, a_sig, N)
            B = model.create_code(a, a_sig, N)
            C = model.create_code(a, a_sig, N)
            # Compute composite code A + B + C
            D[:,i] = model.compute3(A, B, C)
        ...
        for i in range(N_2):
            correlation_coefficient, p_value = pearsonr(D[N_2,:], D[N_2+1+i,:])
